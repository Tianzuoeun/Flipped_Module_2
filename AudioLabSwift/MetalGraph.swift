...
class MetalGraph {
    ...
    // create a new MTLBuffer for the 20-points array graph
    private var shortDataBuffer: MTLBuffer?
      ...
      autoreleasepool {
        self.render()
          ...
          guard let renderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) else {
              ...
              // Draw new buffer
              if let shortDataBuffer = shortDataBuffer {
                  renderEncoder.setVertexBuffer(shortDataBuffer, offset: 0, index: 0)
                  renderEncoder.drawPrimitives(type: .lineStrip, vertexStart: 0, vertexCount: 20)
              }
    ...
    //update the data of the 20-points arrary
    func updateShortData(_ shortData: [Float32]) {
        shortDataBuffer = device.makeBuffer(bytes: shortData, length: shortData.count * MemoryLayout<Float32>.stride, options: [])
        needsRender = true
    }
